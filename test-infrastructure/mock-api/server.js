  ...
  setTimeout(async () => {
    switch (responseMode) {
      case 'error':
        return res
          .status(500)
          .json({ error: 'Internal server error (simulated)' })
      case 'timeout':
  ...
  handleResponseMode(req, res, async () => {
    ...
    if (costCenter.users.find(user => user.login === username)) {
      return res
        .status(409)
        .json({
          error: `User '${username}' already exists in cost center '${name}'`
        })
    }
...
app.delete('/cost-centers/:name/users/:username', (req, res) => {
  console.log(
    `DELETE /cost-centers/${req.params.name}/users/${req.params.username}`
  )
  ...
  handleResponseMode(req, res, async () => {
    ...
    const userIndex = costCenter.users.findIndex(
    ...
    if (userIndex === -1) {
      return res
        .status(404)
        .json({
          error: `User '${username}' not found in cost center '${name}'`
        })
    }
